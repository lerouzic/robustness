...
pdf("figK.pdf", width=10, height=12)
	...
	allplots(list.sim, what="fitness", ylim=c(0.8,1))
	legend("topleft", pch=pchs, col=cols, legend=names(cols))
	allplots(list.sim, what="initenv", lwd.highlight="sim.iev", ylim=c(-27, -22))
	allplots(list.sim, what="lateenv", lwd.highlight="sim.lev", ylim=c(-9, -6))
	allplots(list.sim, what="initmut", ylim=c(-7, -6.5))
	allplots(list.sim, what="latemut", lwd.highlight="sim.som", ylim=c(-7.5, -7))
	allplots(list.sim, what="stability", ylim=c(-26, -20))
dev.off()
