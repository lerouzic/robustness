...
torun.mut <- lapply(mut.values, function(mm) 
	function() pure.run.reps(W0, list(s=s, G=G, N=N, rep=test.rep, summary.every=G, mut.rate=mm), 
		reps=reps, series.name=paste0("real-ref-m", signif(mm, digits=2)), force.run=force.run))
torun.N <- lapply(N.values, function(nn) 
	function() pure.run.reps(W0, list(s=s, G=G, N=nn, rep=test.rep, summary.every=G, mut.rate=mut.rate), 
		reps=reps, series.name=paste0("real-ref-N", nn), force.run=force.run))
torun.genes <- lapply(genes.values, function(gg) 
	function() pure.run.reps(matrix(rnorm(gg^2, sd=0.000001), ncol=gg), list(s=s, G=G, N=N, rep=test.rep, summary.every=G, mut.rate=mut.rate), 
		reps=reps, series.name=paste0("real-ref-g", gg), force.run=force.run))
torun.selg <- lapply(selg.values, function(sg)
	function() pure.run.reps(W0, list(s=c(rep(10,sg),rep(0,n.genes-sg)), G=G, N=N, rep=test.rep, summary.every=G, mut.rate=mut.rate), 
		reps=reps, series.name=paste0("real-ref-sg", sg), force.run=force.run))
torun.sel <- lapply(s.values, function(ss)
	function() pure.run.reps(W0, list(s=c(rep(ss,sel.genes),rep(0,n.genes-sel.genes)), G=G, N=N, rep=test.rep, summary.every=G, mut.rate=mut.rate), 
		reps=reps, series.name=paste0("real-ref-s", signif(ss, digits=2)), force.run=force.run))	
